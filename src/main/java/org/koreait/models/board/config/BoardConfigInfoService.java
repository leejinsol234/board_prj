...
public class BoardConfigInfoService {
    ...
    public ListData<Board> getList(BoardSearch search){ //페이지 번호, 검색 키워드로 게시글 검색하기
        BooleanBuilder andBuilder = new BooleanBuilder();

        int page = Utils.getNumber(search.getPage(),1); //1미만의 수일 경우 기본값1
        ...
        if(StringUtils.hasText(skey)){
            skey = skey.trim(); //공백 제거하고 검색되도록
            if(sopt.equals("bId")){ //게시판 아이디 조회
        ...
        // 글쓰기 권한
        List<BoardAuthority> authorities = search.getAuthority() == null ? null : search.getAuthority().stream().map(BoardAuthority::valueOf).toList(); //문자열을 enum 상수로
